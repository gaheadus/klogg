...
public:
    ...
    public:
        ...
        static constexpr size_t BufferSize = 5 * 1024 * 1024; // IndexingBlockSize = 5MB
        ...
        {
            // Check if buffer belongs to pool using pointer comparison
            for ( size_t i = 0; i < PoolSize; ++i ) {
                if ( pool_[ i ] == buffer ) {
                    buffer->clear();
                    // Push to free list head
                    nextFree_[ i ] = freeHead_;
                    freeList_[ i ] = buffer;
                    freeHead_ = i;
                    ++freeCount_;
                    return;
